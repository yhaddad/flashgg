...
for i in ${rms_cut[@]};do
    ...
    echo " #######################################"
    fggRunJobs.py --load data_76x.json -d /vols/cms04/yhaddad/training-${today}-76x_${label}_data \
		  -x cmsRun standard_vbf_dumper_cfg.py maxEvents=-1 \
		  -q hepshort.q --no-use-tarball useAAA=1 targetLumi=1.00e+3 forwardJetRMSCut=0.03 runOnZee=false \
