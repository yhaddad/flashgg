...
for i in ${rms_cut[@]};do
	...
	then
	    ...
	    echo " ############################### "
	    fggRunJobs.py --load ZPlusJetDump.json -d /vols/cms04/yhaddad/validation-${today}-76x_${label}_rms_${i} \
			  -x cmsRun standard_vbf_dumper_cfg.py maxEvents=-1 \
			  -q hepshort.q --no-use-tarball useAAA=1 targetLumi=1.00e+3 forwardJetRMSCut=$i runOnZee=$run_on_zee \
	    ...
	    echo " ############################### "
	    fggRunJobs.py --load training_part${j}.json -d /vols/cms04/yhaddad/training-${today}-76x_${label}_rms_${i}_p${j}\
			  -x cmsRun standard_vbf_dumper_cfg.py maxEvents=-1 \
