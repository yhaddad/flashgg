...
do 
    today=`date +%F`
    outdir=/vols/cms/yhaddad/VBF-Tree-Moriond17/pujid-moriod17-${wp}-${today}
    fggRunJobs.py --load ${json} -d ${outdir} \
		  -x cmsRun moriond_vbf_dumper_cfg.py maxEvents=-1 runOnZee=False forwardJetRMSCut=0.03 \
		  -q hepmedium.q --no-use-tarball useAAA=1 targetLumi=1.00e+3 pujidWP="none" \
		  lumiMask=${lumiMask} \
