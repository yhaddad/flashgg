...
namespace flashgg {
  ...
  void VBFMVAProducer::produce( Event & evt, const EventSetup & ) {
    ...
    for (unsigned int candIndex =0; candIndex < diPhotonPointers.size() ; candIndex++){
      ...
      //std::cout << " jetPointersDz.size() == " <<jetPointersDz.size() << std::endl;
    ...
    //std::cout << " diPhotonPointers.size()==" << diPhotonPointers.size() << std::endl;
    //std::cout << " vbf_results.size() =="     << vbf_results->size()     << std::endl;
    evt.put(vbf_results);
  }// produce
}// namespace flashgg
